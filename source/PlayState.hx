...
class PlayState extends MusicBeatState
{
	...
	var bruh:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('dave/redsky'));
	var bruhlol:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('dave/redsky_insanity'));
	...
	var the3DWorldEffect:WiggleEffect;
	var the3DWorldEffectWavy:WiggleEffect;
	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				...
				case 'bonus-song' | 'glitch':
					curStage = 'houseNight';
				case 'blocked' | 'corn-theft' | 'secret':
					curStage = 'farmDay';
				case 'maze':
					curStage = 'farmSunset';
				case 'splitathon' | 'mealie' | 'supplanted' | 'screwed':
					curStage = 'farmNight';
		...
		switch (curStage)
		{
			...
			bruhlol.loadGraphic(Paths.image('dave/redsky_insanity'));
			bruhlol.antialiasing = true;
			bruhlol.scrollFactor.set(0.6, 0.6);
			bruhlol.active = true;
			...
			bruhlol.shader = testshader.shader;
			curbg = bruhlol;
			#end
...
switch (daSong)
{
	...
	case 'senpai' | 'roses' | 'thorns' | 'house' | 'insanity' | 'polygonized':
